...
GLuint glOffscreenBB = 0;
static GLuint glOffscreenBBColor = 0;
static GLuint glOffscreenBBDepthStencil = 0;
...
idCVar r_fullscreenDesktop( "r_fullscreenDesktop", "1", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_BOOL, "1: 'real' fullscreen mode 1: keep resolution 'desktop' fullscreen mode" );
idCVar r_customWidth( "r_customWidth", "3840", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_INTEGER, "custom screen width. set r_mode to -1 to activate" );
...
idCVar r_swapInterval( "r_swapInterval", "1", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_INTEGER, "changes wglSwapIntarval (v-sync)" );
...
//BC ATI video cards have issues with r_clear being a non-zero value.
idCVar r_clear( "r_clear", "0", CVAR_RENDERER, "force screen clear every frame, 1 = purple, 2 = black, 'r g b' = custom" );
...
static const char* severityString[] =
{
	"High",
	"Medium",
	"Low",
...
{
	// unsupported values.
	if (_source > GL_DEBUG_SOURCE_OTHER_ARB
		|| _type > GL_DEBUG_TYPE_OTHER_ARB
		|| _severity > GL_DEBUG_SEVERITY_LOW)
	{
		return;
	}

	// NOTE: re-index this so it fits in the string lookup.
	if (_severity == GL_DEBUG_SEVERITY_NOTIFICATION)
	{
#if 1//def ENABLE_GL_NOTIFICATIONS
		_severity = GL_DEBUG_SEVERITY_HIGH_ARB+3;
#else
		return;
#endif
	}
	...
	// Errors to ignore
	if (_id == 131185 // Buffer detailed info
		)
	{
		return;
	}

	common->Printf("OpenGL[%d][%s][%s][%s]: %s\n", _id,
		severityString[_severity-GL_DEBUG_SEVERITY_HIGH_ARB],
		typeString[_type-GL_DEBUG_TYPE_ERROR_ARB],
		sourceString[_source-GL_DEBUG_SOURCE_API_ARB],
		_message);

	SDL_assert(_type != GL_DEBUG_TYPE_ERROR_ARB);
}
...
static void R_CheckPortableExtensions( void ) {
	...
	// GL_EXT_texture
	glConfig.textureAvailable = R_CheckExtension( "GL_EXT_texture" );
	...
	if ( glConfig.framebufferObjectAvailable ) {
		qglIsRenderbufferEXT = (PFNGLISRENDERBUFFEREXTPROC)GLimp_ExtensionPointer( "glIsRenderbufferEXT" );
		...
		qglGetRenderbufferParameterivEXT = (PFNGLGETRENDERBUFFERPARAMETERIVEXTPROC)GLimp_ExtensionPointer( "glGetRenderbufferParameterivEXT" );
		qglIsFramebufferEXT = (PFNGLISFRAMEBUFFEREXTPROC)GLimp_ExtensionPointer( "glIsFramebufferEXT" );
		...
		qglFramebufferTexture1DEXT = (PFNGLFRAMEBUFFERTEXTURE1DEXTPROC)GLimp_ExtensionPointer( "glFramebufferTexture1DEXT" );
		qglFramebufferTexture2DEXT = (PFNGLFRAMEBUFFERTEXTURE2DEXTPROC)GLimp_ExtensionPointer( "glFramebufferTexture2DEXT" );
		qglFramebufferTexture3DEXT = (PFNGLFRAMEBUFFERTEXTURE3DEXTPROC)GLimp_ExtensionPointer( "glFramebufferTexture3DEXT" );
		qglFramebufferRenderbufferEXT = (PFNGLFRAMEBUFFERRENDERBUFFEREXTPROC)GLimp_ExtensionPointer( "glFramebufferRenderbufferEXT" );
		qglGetFramebufferAttachmentParameterivEXT = (PFNGLGETFRAMEBUFFERATTACHMENTPARAMETERIVEXTPROC)GLimp_ExtensionPointer( "glGetFramebufferAttachmentParameterivEXT" );
	...
	if ( glConfig.framebufferBlitAvailable ) {
		qglBlitFramebufferEXT = (PFNGLBLITFRAMEBUFFEREXTPROC)GLimp_ExtensionPointer( "glBlitFramebufferEXT" );
	}

	// GL_EXT_packed_depth_stencil
	glConfig.packedDepthStencilAvailable = R_CheckExtension( "GL_EXT_packed_depth_stencil" );

	// GL_ARB_debug_output
	glConfig.debugOutputAvailable = R_CheckExtension( "GL_ARB_debug_output" ) && R_CheckExtension( "KHR_debug" );
...
vidmode_t r_vidModes[] = {
	...
	{ "Mode  2: 856x480",		856,	480 },
	{ "Mode  3: 960x720",		960,	720 },
	{ "Mode  4: 1024x480",		1024,	480 },
    { "Mode  5: 1024x768",		1024,	768 },
    { "Mode  6: 1152x864",		1152,	864 },
	{ "Mode  7: 1280x720",		1280,	720 },
	{ "Mode  8: 1280x768",		1280,	768 },
	{ "Mode  9: 1280x800",		1280,	800 },
	{ "Mode  10: 1280x960",		1280,	960 },
    { "Mode  11: 1280x1024",	1280,	1024 },
	{ "Mode  12: 1360x768",		1360,	768 },
	{ "Mode  13: 1366x768",		1366,	768 },	
	{ "Mode  14: 1400x1050",	1400,	1050 },	
	{ "Mode  15: 1440x900",		1440,	900 },
	{ "Mode  16: 1600x900",		1600,	900 },
    { "Mode  17: 1600x1200",	1600,	1200 },
	{ "Mode  18: 1680x1050",	1680,	1050 },
	{ "Mode  19: 1920x1080",	1920,	1080 },
	{ "Mode  20: 1920x1200",	1920,	1200 },
	{ "Mode  21: 1920x1440",	1920,	1440 },
	{ "Mode  22: 2048x1536",	2048,	1536 },
	{ "Mode  23: 2560x1440",	2560,	1440 },
	{ "Mode  24: 2560x1600",	2560,	1600 },
	{ "Mode  25: 2880x1800",	2880,	1800 },
	{ "Mode  26: 3200x1080",	3200,	1080 },
	{ "Mode  27: 3840x1080",	3840,	1080 },
};
...
static bool R_GetModeInfo( int *width, int *height, int mode ) {
	...
	// If we get an out of bounds mode then default to desktop resolution.
#ifdef USE_SDL
...
void R_InitOpenGL( void ) {
	...
	// create an off screen backbuffer if window != vidDims
	if (glConfig.framebufferObjectAvailable && glConfig.framebufferBlitAvailable && glConfig.packedDepthStencilAvailable
		&& glConfig.vidWidth != glConfig.winWidth || glConfig.vidHeight != glConfig.winHeight) {
		qglGenFramebuffersEXT(1, &glOffscreenBB);
		qglBindFramebufferEXT(GL_FRAMEBUFFER_EXT, glOffscreenBB);

		// setup color and depth targets.
		qglGenRenderbuffersEXT(1, &glOffscreenBBColor);
		qglBindRenderbufferEXT(GL_RENDERBUFFER, glOffscreenBBColor);
		qglRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL_RGBA8_EXT, glConfig.vidWidth, glConfig.vidHeight);
		qglFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_RENDERBUFFER, glOffscreenBBColor);

		qglGenRenderbuffersEXT(1, &glOffscreenBBDepthStencil);
		qglBindRenderbufferEXT(GL_RENDERBUFFER, glOffscreenBBDepthStencil);
		qglRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL_DEPTH24_STENCIL8_EXT, glConfig.vidWidth, glConfig.vidHeight);
		qglFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, glOffscreenBBDepthStencil);

		qglReadBuffer(GL_COLOR_ATTACHMENT0_EXT);
		qglDrawBuffer(GL_COLOR_ATTACHMENT0_EXT);

		GLenum fbStatus = qglCheckFramebufferStatusEXT(GL_FRAMEBUFFER);
		//The configuration of the frame buffer is good, but unsupported
		if (fbStatus == GL_FRAMEBUFFER_UNSUPPORTED_EXT)
		{
			common->Printf("Frame buffer is not complete! Format is unsupported!\n");
		}
		//The configuration of the frame buffer is bad
		else if (fbStatus != GL_FRAMEBUFFER_COMPLETE_EXT)
		{
			switch(fbStatus)
			{
			case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT_EXT:
				common->Printf("Frame buffer is not complete! Incomplete attachments!\n");
				break;
			case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT_EXT:
				common->Printf("Frame buffer is not complete! Missing attachments!\n");
				break;
			case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS_EXT:
				common->Printf("Frame buffer is not complete! Incomplete dimensions!\n");
				break;
			case GL_FRAMEBUFFER_INCOMPLETE_FORMATS_EXT:
				common->Printf("Frame buffer is not complete! Incomplete formats!\n");
				break;
			case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER:
				common->Printf("Frame buffer is not complete! Incomplete draw buffer!\n");
				break;
			case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER:
				common->Printf("Frame buffer is not complete! Incomplete read buffer!\n");
				break;
			default:
				common->Printf("Frame buffer is not complete! Unknown error!\n");
				break;
			};
...
#ifdef _DEBUG // Install the message callback for debug builds.
	if ( glConfig.debugOutputAvailable ) {
		qglDebugMessageCallbackARB(glErrorCallback, NULL);
...
void R_VidRestart_f( const idCmdArgs &args ) {
	...
	if ( full ) {
		...
		if (glOffscreenBB > 0) {
			qglDeleteRenderbuffersEXT(1, &glOffscreenBBColor);
			qglDeleteRenderbuffersEXT(1, &glOffscreenBBDepthStencil);
			qglDeleteFramebuffersEXT(1, &glOffscreenBB);
			glOffscreenBB = glOffscreenBBColor = glOffscreenBBDepthStencil = 0;
		}
